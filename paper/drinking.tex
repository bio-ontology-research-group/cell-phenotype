...
process attributes that are linked directly to processes, attributes
that arise due to process parts and attributes of processes, and
attributes arising from attributes of process participants and other
process attributes.

Attribute that can be directly linked to a process are the result of
the temporal extension of a process and include process {\em duration}
and {\em temporal location}. A duration is an attribute that
characterizes the temporal extent of a process and is similar to {\em
  Length}, {\em Area} and {\em Volume} for one-, two- and
three-dimensional entities. A {\em Temporal location} positions the
time interval during which the process occurs with respect to a
