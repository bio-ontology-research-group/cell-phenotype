...
\author[Hoehndorf et al.]{Robert Hoehndorf$^1$ and Gabriella
  Rustici$^2$ and Midori Harris(?)$^3$ and Georgios V. Gkoutos$^{1}$}
...
% A phenotype such as {\em Increased drinking behavior} is used for the
% annotation of phenotypes in mouse and other species.

% Different meanings:
% \begin{itemize}
% \item amount of substance increased over fixed period (e.g., 24h)
% \item amount of substance per drinking act is increased
% \item amount of time spent drinking over fixed period is increased
% \item amount of time spent per drinking act is increased
% \item number of drinking acts per fixed period increased
% \item variety of substances that organism drinks is increased (over
%   24h, per drinking act)
% \item substance flow during drinking increased
% \end{itemize}

% more than just these aspects of ``drinking behavior'' can be increased
...
conceptualization of a domain of knowledge \cite{Gruber1995,
  Guarino1998}.  % In philosophy, on the other hand, an ontology is a
% system of categories that account for a particular view on the
% world. The philosophical sense of ``ontology'' is what computer
% science calls ``conceptualization''.  While a conceptualization is
% language-independent, its specification is based on a language. In
% this sense, ontologies specify some aspects of the meaning of terms
% in vocabularies.
...
In English language, we may say that an object has a {\em frequency}
or {\em rate} as if these were attributes of objects at time
points. For example, that my heart {\em now} beats with a frequency of
80 beats per minute, or that a car is moving with a speed of 280
kilometres per minute {\em at a particular point in time} (e.g., as
observed with a speed camera) may be considered attributes of the
objects, not attributes of processes. In our analysis, rates as
attributes of objects can be explicitly defined using rates of
processes. For example, the heart beating frequency of a particular
heart $h$ at a time point $t$ is the frequency of heart beating
processes in which $h$ participates at an infinitesimally small
process around $t$ and within a reference process. The reference
process is necessary in order to obtain a value for a frequency or
rate even if the process in question is not occurring. For example, a
rate of cardiac muscle contraction can be determined even when no
cardiac muscle contraction is occurring in virtue of a reference
process with a rate of cardiac muscle contraction.
...
\subsection{Cell Phenotype Ontology: implementation and evaluation}
...
In the CPO, identifiers are of the form {\tt CPO:XXGGGGGGG}, where
{\tt GGGGGGG} is the seven-digit identifier of the GO class on which
the CPO class is based, and {\tt XX} is a prefix that identifies the
type of pattern we applied to the GO class. For example, based on the
class {\em Apoptotis} ({\tt GO:0006915}), we generate the CPO classes
{\em Abnormality of Apoptosis}, {\em Abnormality of single occurrence
  of apoptosis} and {\em Abnormality of regulation of apoptosis}.
We use the prefixes {\tt 12}, {\tt 14} and {\tt 15} for each of the
corresponding phenotype patterns, and consequently generate the class
identifiers {\tt CPO:120006915}, {\tt CPO:140006915} and {\tt
  CPO:150006915}. 
